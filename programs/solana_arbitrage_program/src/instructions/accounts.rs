...
#[derive(Accounts)]
pub struct ExecutePumpArb<'info> {
    #[account(mut)]
    pub user: Signer<'info>,

    #[account(
        seeds = [Operator::SEED, user.key().as_ref()],
        bump = operator.bump,
    )]
    ...
    #[account(mut)]
    pub pump_pool_base_vault: UncheckedAccount<'info>,

    /// CHECK: pool quote vault
    #[account(mut)]
    pub pump_pool_quote_vault: UncheckedAccount<'info>,
    ...
    #[account(mut)]
    pub pump_protocol_fee_recipient_ata: UncheckedAccount<'info>,
    ...
    /// CHECK: extra writable account A — buy #24 / sell #22
    #[account(mut)]
    pub pump_extra_account_a: UncheckedAccount<'info>,

    /// CHECK: extra writable account B — sell #23
    #[account(mut)]
    pub pump_extra_account_b: UncheckedAccount<'info>,

    /// CHECK: static readonly account at end of buy/sell
    pub pump_static_account: UncheckedAccount<'info>,
}

#[derive(Accounts)]
pub struct ExecuteMeteoraArb<'info> {
    #[account(mut)]
    pub user: Signer<'info>,
    ...
    /// CHECK: BinArrayBitmapExtension PDA — seeds: ["bitmap", lb_pair]
    pub meteora_bin_array_bitmap_extension: UncheckedAccount<'info>,

    /// CHECK: Reserve X token account (LIA vault of the pool)
    #[account(mut)]
    pub meteora_reserve_x: UncheckedAccount<'info>,

    /// CHECK: Reserve Y token account (WSOL vault of the pool)
    #[account(mut)]
    pub meteora_reserve_y: UncheckedAccount<'info>,

    /// CHECK: Oracle PDA — seeds: ["oracle", lb_pair]
    #[account(mut)]
    pub meteora_oracle: UncheckedAccount<'info>,

    /// CHECK: Host fee recipient ATA — pass Meteora program ID when no host fee
    pub meteora_host_fee_in: UncheckedAccount<'info>,

    /// CHECK: Event authority PDA — seeds: ["__event_authority"]
    pub meteora_event_authority: UncheckedAccount<'info>,

    /// CHECK: Memo program (required by swap2 for Token-2022)
    pub meteora_memo_program: UncheckedAccount<'info>,

    /// CHECK: Meteora DLMM program
    pub meteora_program: UncheckedAccount<'info>,
    // Bin arrays are passed via ctx.remaining_accounts
}
